/* =====================================================================
   ...
   Ported from ThreeUI's Kage landing page (public/landing-pages/kage.html).
   Everything here is generated at runtime: no photographs, no video, no
   external assets beyond three.js itself. The authored page's own note is
   what makes this port possible — the fourteen .webp files it ships are
   page decoration (card stills and DOM foreground cut-outs), not scene
   input, and buildForeground() below regenerates equivalent cut-outs
   procedurally, exactly as the source does.

   What is kept: every texture generator, every world builder, the light
   rig, the atmosphere, the leaf fall, the pointer wisps, the bloom chain
   and the composite grade, and the camera spline.

   What is dropped: the page chrome the original wraps around the scene —
   its preloader, nav, reveal system, card viewports, and the 3-D wordmark
   (which needs the subset font the bundle embeds as base64). None of that
   is scene; all of it is Kage's own marketing layout, which this site
   replaces with its own.

   The one structural change: the source drives RIG.prog from its own
   [data-cam] sections. Here the host passes progress in, so the camera is
   keyed off this site's [data-audit-section] chapters instead.

   This file is deliberately plain JS. The original is written against
   loose array indexing throughout, and `noUncheckedIndexedAccess` would
   turn a faithful port into fifteen hundred lines of non-null assertions.
   Types live alongside it in kageRenderer.d.ts.
   ===================================================================== */
...
function fbm(n, x, y, oct, lac, gain) {
  ...
  for (let i = 0; i < (oct || 4); i++) { s += a * n(x * f, y * f); m += a; a *= (gain || 0.5); f *= (lac || 2); }
  return s / m;                                    /* -1 … 1 */
}
...
/* stacked, up-scaled value noise — 30x faster than a per-pixel fbm and
...
function texWall() {
  ...
  /* mottling */
  x.globalCompositeOperation = 'overlay'; x.globalAlpha = 0.82;
  ...
  for (let i = 0; i < 6; i++) for (let j = 0; j < 4; j++) {
    ...
    g.addColorStop(0, 'rgba(0,0,0,.5)'); g.addColorStop(0.7, 'rgba(0,0,0,.18)'); g.addColorStop(1, 'rgba(0,0,0,0)');
  ...
  /* fine tooth */
  x.globalAlpha = 0.16; x.globalCompositeOperation = 'overlay';
...
/* --------------------------------------------------- temple timber · cedar
   ...
   none of them is fbm:
     ...
     1 · ring geometry. A plank is a flat cut through a round log, so the
         ...
         x = pith +/- sqrt(r^2 - d(y)^2), with the cut depth d wandering down
         the board — hands over the arches for free.
     2 · the boards themselves. Joints, and a different tone on either side
         of one. A wall with no joint in it is a wall of plastic.
     3 · pores, so the face has tooth rather than a painted finish.

   boards:0 gives a single timber for the columns, sills and rails, which
   are each one stick of wood and must not carry a joint. */
function texWood(seed, opt) {
  ...
  /* board layout — widths jittered, then normalised so the run closes on
     the tile edge and the map still wraps */
  const nb = o.boards === undefined ? 7 : o.boards;
  const cuts = [0];
  if (nb > 0) {
    const ws = []; let sum = 0;
    for (let i = 0; i < nb; i++) { const v = 0.70 + rnd() * 0.60; ws.push(v); sum += v; }
    let acc = 0;
  ...
  for (let b = 0; b < cuts.length - 1; b++) {
    ...
    const pith = x0 + bw * (rnd() * 2.8 - 0.9);        /* usually outside it */
    const d0 = bw * (0.12 + rnd() * 1.7);              /* how deep the cut ran */
    const amp = bw * (0.10 + rnd() * 0.40);
      ...
      for (const side of [-1, 1]) {
        ...
        const flush = () => {
          ...
          stroke(pts.map((q) => [q[0] + side * (wide + 0.6), q[1]]), x,
                 'rgba(150,120,96,' + (dark * 0.30).toFixed(2) + ')', wide * 0.7);
          pts = [];
        };
    ...
    for (let i = 0; i < 220; i++) {
      ...
      x.fillStyle = 'rgba(0,0,0,' + (0.10 + rnd() * 0.22) + ')';
      x.fillRect(px2, py, 0.7 + rnd() * 0.7, ln);
      hx.fillStyle = 'rgba(0,0,0,.22)'; hx.fillRect(px2, py, 0.8, ln);
    }
    /* the odd knot, where a branch left the trunk */
    if (rnd() > 0.55) {
      const kx = x0 + bw * (0.2 + rnd() * 0.6), ky = rnd() * H, kr = 2.5 + rnd() * 5;
      for (let i = 5; i >= 1; i--) {
        const t = i / 5;
        x.strokeStyle = 'rgba(0,0,0,' + (0.5 - t * 0.25).toFixed(2) + ')'; x.lineWidth = 1.1;
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const pts = [];
      for (let y = sy; y < sy + ln; y += 6) pts.push([sx + Math.sin(y * 0.07) * 1.4, y]);
      stroke(pts, x, 'rgba(0,0,0,.62)', 0.7 + rnd() * 1.2);
      stroke(pts, hx, 'rgba(0,0,0,.55)', 0.7 + rnd() * 1.2);
    }
  ...
  /* soot, running down the boards rather than pooling in clouds — this is
     the one weathering pass, and it follows the grain because water does */
  for (let i = 0; i < 90; i++) {
    const sx = rnd() * W, top = rnd() * H, ln = H * (0.15 + rnd() * 0.5);
    const g = x.createLinearGradient(0, top, 0, top + ln);
    g.addColorStop(0, 'rgba(0,0,0,0)');
    g.addColorStop(0.3, 'rgba(0,0,0,' + (0.10 + rnd() * 0.16) + ')');
    g.addColorStop(1, 'rgba(0,0,0,0)');
    x.fillStyle = g; x.fillRect(sx, top, 0.8 + rnd() * 3.4, ln);
  }
...
/* ------------------------------------------------------------ cut granite
   ...
   grain and wear —
     ...
     1 · the speckle. Three mineral populations at three tones: pale
         feldspar, mid quartz, black mica. A granite without visible crystal
         is grey plastic, and it is the one thing a colour ramp cannot fake.
     2 · the chisel. Parallel bruising left by a point on a dressed face,
         each mark a dark gouge with a lit shoulder below it.
     3 · pitting and fracture, and moss keyed to *those* — moss grows where
         water sits, which is the low ground, so it is read out of the
         height field rather than dropped on as its own cloud. */
function texStone(seed, opt) {
  ...
  const speck = (n, fill, smin, smax, hgt) => {
    for (let i = 0; i < n; i++) {
      ...
      x.beginPath(); x.ellipse(px, py, s, s * (0.55 + rnd() * 0.85), rnd() * TAU, 0, TAU); x.fill();
      if (hgt) {
        hx.fillStyle = hgt;
        hx.beginPath(); hx.ellipse(px, py, s, s * 0.8, 0, 0, TAU); hx.fill();
      }
    }
  };
  speck(3400, (q) => 'rgba(206,210,204,' + (0.06 + q() * 0.20) + ')', 0.6, 2.8, 'rgba(255,255,255,.13)');
  speck(2000, (q) => 'rgba(126,136,134,' + (0.07 + q() * 0.20) + ')', 0.8, 3.4, null);
  speck(1300, (q) => 'rgba(8,10,12,'     + (0.14 + q() * 0.38) + ')', 0.5, 2.4, 'rgba(0,0,0,.16)');

  /* 2 · the chisel */
  for (let i = 0; i < 220; i++) {
    const py = rnd() * H, px = rnd() * W, len = 5 + rnd() * 24, ang = -0.26 + rnd() * 0.52;
  ...
  for (let i = 0; i < 260; i++) {
    const px = rnd() * W, py = rnd() * H, rr = 0.8 + rnd() * rnd() * 5;
    const g = x.createRadialGradient(px - rr * 0.3, py - rr * 0.3, rr * 0.1, px, py, rr);
    g.addColorStop(0, 'rgba(0,0,0,.42)'); g.addColorStop(0.7, 'rgba(0,0,0,.16)');
    g.addColorStop(1, 'rgba(198,204,198,.10)');
    x.fillStyle = g; x.beginPath(); x.arc(px, py, rr, 0, TAU); x.fill();
    hx.fillStyle = 'rgba(0,0,0,.40)'; hx.beginPath(); hx.arc(px, py, rr * 0.8, 0, TAU); hx.fill();
  }
  for (let i = 0; i < 16; i++) {
    let px = rnd() * W, py = rnd() * H, a = rnd() * TAU;
    const pts = [[px, py]];
    for (let k = 0; k < 24; k++) {
      a += (rnd() - 0.5) * 0.9; px += Math.cos(a) * 9; py += Math.sin(a) * 9;
      pts.push([px, py]);
    }
    [[x, 'rgba(0,0,0,.46)'], [hx, 'rgba(0,0,0,.55)']].forEach(([d, st]) => {
      d.beginPath(); d.moveTo(pts[0][0], pts[0][1]);
      pts.forEach((q) => d.lineTo(q[0], q[1]));
      d.strokeStyle = st; d.lineWidth = 0.7 + rnd() * 0.9; d.stroke();
    });
  }
  ...
  for (let i = 0; i < W * H; i++) {
    const t = clamp((0.5 - hd[i * 4] / 255) * 4.2, 0, 1) * amt;
    if (t <= 0) continue;
    const j = i * 4;
    im.data[j]     = lerp(im.data[j],     30, t * 0.75);
    im.data[j + 1] = lerp(im.data[j + 1], 44, t * 0.95);
    im.data[j + 2] = lerp(im.data[j + 2], 28, t * 0.85);
    ...
    rm.data[j] = rm.data[j + 1] = rm.data[j + 2] = lerp(rm.data[j], 255, t * 0.5);
  }
...
/* ------------------------------------------- weathered vermilion · the gate
   ...
   weather: the lacquer has crazed into a fine net, flaked off the edges back
   to the black wood underneath, and run in streaks where the rain came down
   it. So the wood is drawn first and the paint is laid over it at less than
   full coverage — the grain has to read *through* the red, which is what
   stops it looking like plastic. */
function texLacquer() {
  const W = 512, H = 512;
  ...
  /* the coat */
  x.globalAlpha = 0.80; x.fillStyle = '#6f3227'; x.fillRect(0, 0, W, H);
  x.globalAlpha = 1;
  ...
  for (let i = 0; i < 190; i++) {
    let px = rnd() * W, py = rnd() * H, a = rnd() * TAU;
    x.strokeStyle = 'rgba(24,8,6,' + (0.28 + rnd() * 0.4) + ')';
    hx.strokeStyle = 'rgba(0,0,0,.42)';
    x.lineWidth = hx.lineWidth = 0.5 + rnd() * 0.7;
    x.beginPath(); hx.beginPath(); x.moveTo(px, py); hx.moveTo(px, py);
    for (let k = 0; k < 5 + rnd() * 9; k++) {
      a += (rnd() - 0.5) * 1.5; px += Math.cos(a) * (5 + rnd() * 9); py += Math.sin(a) * (5 + rnd() * 9);
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const pts = [];
    for (let k = 0; k < 9; k++) {
      const a = k / 9 * TAU, rr = rad * (0.55 + rnd() * 0.75);
  ...
  for (let i = 0; i < 120; i++) {
    const sx = rnd() * W, w = 0.6 + rnd() * 2.6, top = rnd() * H, len = H * (0.2 + rnd() * 0.6);
    const g = x.createLinearGradient(0, top, 0, top + len);
    g.addColorStop(0, 'rgba(0,0,0,0)');
    g.addColorStop(0.3, rnd() > 0.5 ? 'rgba(12,4,4,.24)' : 'rgba(210,150,120,.05)');
    g.addColorStop(1, 'rgba(0,0,0,0)');
    x.fillStyle = g; x.fillRect(sx, top, w, len);
  }
  rx.globalAlpha = 0.6; rx.globalCompositeOperation = 'multiply';
  rx.drawImage(fbmCanvas(W, H, 401, 4, 6, 1.2), 0, 0);
  rx.globalAlpha = 1; rx.globalCompositeOperation = 'source-over';
  return { map: c, normal: normalFromHeight(h, 2.2), rough: r };
}

/* the shoji screen the lamps burn through */
function texShoji() {
...
function texLeaf() {
  ...
  for (let i = 0; i < lobes; i++) {
    ...
    const len = i === 2 ? 0.96 : (i === 1 || i === 3 ? 0.82 : 0.60);
    const wob = 0.17;
    x.moveTo(0, 0.02);
  ...
  x.lineWidth = 0.05; x.strokeStyle = '#fff'; x.stroke();
...
function texSky() {
  ...
  g.addColorStop(0, 'rgb(7,11,18)');     g.addColorStop(0.34, 'rgb(12,20,34)');
  g.addColorStop(0.66, 'rgb(16,26,40)'); g.addColorStop(0.88, 'rgb(22,34,50)');
  g.addColorStop(1, 'rgb(13,21,32)');
  x.fillStyle = g; x.fillRect(0, 0, W, H);
  /* cloud, and a low cold bloom off the valley behind the ridge */
  ...
  wg.addColorStop(0, 'rgba(52,104,150,.30)'); wg.addColorStop(0.5, 'rgba(40,78,116,.12)');
  wg.addColorStop(1, 'rgba(0,0,0,0)');
  x.fillStyle = wg; x.fillRect(0, 0, W, H);
  /* a scatter of stars, faint enough to survive the bloom */
  const rnd = mulberry32(881);
  for (let i = 0; i < 420; i++) {
    const sx = rnd() * W, sy = rnd() * H * 0.78, r = 0.5 + rnd() * rnd() * 1.7;
...
function texRidge() {
  ...
  for (let i = 0; i <= W; i += 4) {
    const t = i / W;
    const ridge = 0.46 + 0.30 * (fbm(n, t * 2.4, 0.5, 4, 2.1, 0.55) * 0.5 + 0.5)
...
function texRoof() {
  ...
  for (let i = 0; i < ribs; i++) {
    ...
    g.addColorStop(0, 'rgba(0,0,0,.62)');  g.addColorStop(0.30, 'rgba(148,178,192,.13)');
    g.addColorStop(0.66, 'rgba(84,110,124,.05)'); g.addColorStop(1, 'rgba(0,0,0,.62)');
  ...
  for (let y = 0; y < H; y += H / 9) {
    ...
    for (let i = 0; i < 26; i++) {                     /* chipped, mossed edges */
      x.fillStyle = rnd() > 0.5 ? 'rgba(0,0,0,.30)' : 'rgba(96,116,92,.10)';
      x.fillRect(rnd() * W, y + 1, 2 + rnd() * 9, 2 + rnd() * 3);
    }
  }
  x.globalCompositeOperation = 'overlay'; x.globalAlpha = 0.46;
  x.drawImage(fbmCanvas(256, 256, 211, 5, 3, 1), 0, 0, W, H);
  x.globalAlpha = 1; x.globalCompositeOperation = 'source-over';
  hx.globalAlpha = 0.3; hx.drawImage(fbmCanvas(256, 256, 213, 4, 16, 1.2), 0, 0, W, H);
  hx.globalAlpha = 1;
  return { map: c, normal: normalFromHeight(h, 2.2) };
}

/* -------------------------------------------------------- the blood moon
   A real lunar disc, not a glow sprite and not a noise field. Three things
   do the work, in this order of importance:
     ...
     1 · the maria. The dark seas are the Moon's signature and they are not
         ...
         near-side positions and blurred, they are what makes the disc read
         as the Moon at a glance instead of as a planet.
   ...
   sits at G/R ~ B/R ~ .48 — a rose red with green and blue level. Grading
   it as an orange (blue well under green) is what makes a CG moon read as a
   fireball. The disc is authored here in near-neutral albedo and the colour
   is applied once, on the material. */
function texMoon() {
  ...
  g.addColorStop(0, 'rgb(150,150,150)'); g.addColorStop(0.55, 'rgb(158,158,158)');
  g.addColorStop(0.86, 'rgb(178,178,178)'); g.addColorStop(1, 'rgb(196,196,196)');
  ...
  const seas = [
    [-0.52, -0.06, 0.46, 0.80],   /* Oceanus Procellarum */
    [-0.26, -0.38, 0.31, 0.92],   /* Imbrium             */
    [ 0.13, -0.31, 0.20, 0.88],   /* Serenitatis         */
    [ 0.30, -0.08, 0.23, 0.84],   /* Tranquillitatis     */
    [ 0.45,  0.12, 0.15, 0.78],   /* Fecunditatis        */
    [ 0.27,  0.27, 0.12, 0.74],   /* Nectaris            */
    [ 0.57, -0.30, 0.12, 0.95],   /* Crisium, the one that stands on its own */
    [-0.27,  0.30, 0.19, 0.70],   /* Nubium              */
    [-0.47,  0.25, 0.13, 0.72],   /* Humorum             */
  ];
  ...
  [[-0.10, 0.54, 150, 1], [-0.28, -0.07, 80, 0.6], [-0.46, -0.04, 60, 0.45]].forEach(([u, v, n, str]) => {
    const [ox, oy] = px(u, v);
    for (let i = 0; i < n; i++) {
      const a = rnd() * TAU, len = R * (0.30 + rnd() * rnd() * 1.3);
      ...
      rg.addColorStop(0, 'rgba(255,255,255,' + (0.085 * str).toFixed(3) + ')');
      rg.addColorStop(0.4, 'rgba(255,255,255,' + (0.055 * str).toFixed(3) + ')');
      rg.addColorStop(1, 'rgba(255,255,255,0)');
      rx2.strokeStyle = rg; rx2.lineWidth = 0.8 + rnd() * 2.4; rx2.lineCap = 'round';
      rx2.beginPath(); rx2.moveTo(p0[0], p0[1]);
      rx2.quadraticCurveTo(ox + Math.cos(a + 0.06) * len * 0.55, oy + Math.sin(a + 0.06) * len * 0.55,
                           p1[0], p1[1]);
      rx2.stroke();
    }
  });
  x.save(); x.filter = 'blur(2.4px)'; x.globalCompositeOperation = 'lighter';
  x.globalAlpha = 0.62; x.drawImage(ray, 0, 0); x.restore();

  /* --- 3 · the crater field ----------------------------------------------
     A crater is not a ring. Stroked with a gradient running across it, the
     same circle gives a rim lit on the sun side and thrown into shadow on
     the other — which is the read — where a radial ring gives a bubble. */
  const inSea = (u, v) => seas.some(([su, sv, rad]) =>
    Math.hypot(u - su, (v - sv) * 1.15) < rad * 0.82);
  for (let i = 0; i < 620; i++) {
    const a = rnd() * TAU, rr = Math.sqrt(rnd()) * 0.97;
    const u = Math.cos(a) * rr, v = Math.sin(a) * rr;
    /* the seas are young and nearly unmarked — that contrast is the point */
    if (inSea(u, v) && rnd() > 0.12) continue;
    const [cx2, cy] = px(u, v);
    const big = rnd() > 0.975;
    const r = (1 + rnd() * rnd() * rnd() * (big ? 34 : 11)) * (S / 512);
    const fade = 0.55 + 0.45 * Math.sqrt(Math.max(0, 1 - rr * rr));   /* limb falloff */
    /* foreshortened toward the limb, like anything on a sphere */
    const sq = Math.sqrt(Math.max(0, 1 - rr * rr)) * 0.72 + 0.28;
    x.save(); x.translate(cx2, cy); x.rotate(Math.atan2(v, u)); x.scale(sq, 1);
    x.rotate(-Math.atan2(v, u));                       /* keep the sun direction */
    const rimW = Math.max(0.8, r * 0.26);
    const lg = x.createLinearGradient(-r, -r, r, r);   /* sun sits up-left */
    lg.addColorStop(0, 'rgba(255,255,255,' + (0.34 * fade).toFixed(3) + ')');
    lg.addColorStop(0.5, 'rgba(255,255,255,0)');
    lg.addColorStop(1, 'rgba(0,0,0,' + (0.38 * fade).toFixed(3) + ')');
    x.strokeStyle = lg; x.lineWidth = rimW;
    x.beginPath(); x.arc(0, 0, Math.max(0.6, r - rimW * 0.5), 0, TAU); x.stroke();
    if (r > 3) {                                        /* the floor it encloses */
      const fg = x.createLinearGradient(-r, -r, r, r);
      fg.addColorStop(0, 'rgba(0,0,0,' + (0.21 * fade).toFixed(3) + ')');
      fg.addColorStop(1, 'rgba(255,255,255,' + (0.07 * fade).toFixed(3) + ')');
      x.fillStyle = fg; x.beginPath(); x.arc(0, 0, r - rimW, 0, TAU); x.fill();
    }
    if (r > 13) {                                       /* central peak */
      x.fillStyle = 'rgba(255,255,255,' + (0.14 * fade).toFixed(3) + ')';
      x.beginPath(); x.arc(-r * 0.04, -r * 0.04, r * 0.11, 0, TAU); x.fill();
    }
    x.restore();
  }
  ...
  x.globalCompositeOperation = 'overlay'; x.globalAlpha = 0.13;
  x.drawImage(fbmCanvas(S, S, 977, 2, 210, 1.3), 0, 0, S, S);
  x.globalAlpha = 1; x.globalCompositeOperation = 'source-over';

  /* the terminator side: the disc is not lit dead-on */
  const sh = x.createRadialGradient(S * 0.40, S * 0.38, S * 0.18, S * 0.52, S * 0.56, S * 0.72);
...
/* ---------------------------------------------- 3 · the foreground cut-outs
   These are the "transparent PNG" layers: painted at high resolution with a
   ragged alpha edge, then hung in front of the type as real geometry so they
   parallax and sway. The authored page also ships nine .webp cut-outs for
   its DOM foreground; those are page decoration and are not needed here —
   the scene's own near plane is generated below, exactly as the source does. */
function texGrassCutout(seed, opt) {
  ...
  const peak  = opt.peak  !== undefined ? opt.peak  : 0.60;
  const wide  = opt.wide  !== undefined ? opt.wide  : 0.40;
  const prof = new Float32Array(W);
  for (let i = 0; i < W; i++) {
    const t = i / W;
    let m = Math.exp(-Math.pow((t - crest) / wide, 2) * 2.1);
    m += 0.46 * Math.exp(-Math.pow((t - crest - (opt.crest2 || 0.40)) / (wide * 0.62), 2) * 3.1);
    m += 0.30 * Math.exp(-Math.pow((t - crest + (opt.crest3 || 0.46)) / (wide * 0.70), 2) * 3.4);
    const g = fbm(n, t * 4.2, 0.5, 4, 2.05, 0.52) * 0.5 + 0.5;
    prof[i] = m * (0.80 + 0.38 * g);
  }
  ...
  bg.addColorStop(0, '#1a2416'); bg.addColorStop(0.38, '#0e150c'); bg.addColorStop(1, '#040604');
  ...
  /* --- blades ----------------------------------------------------- */
  const LIGHT = opt.light || [-0.42, -0.91];            /* light from up-left */
  ...
  for (let k = 0; k < N; k++) {
    ...
    const by = s + depth * (H - s) + (rnd() - 0.5) * 6;
    if (by > H + 20) continue;
  ...
  for (let k = 0; k < blades.length; k++) {
    const b = blades[k];
    const bx = b.i + (b.r - 0.5) * 5;
    const grow = 1 - 0.52 * b.depth;
    const len = LEN * (0.36 + 1.05 * b.r2 * b.r2) * grow;
    const lean = (b.r3 - 0.5) * 1.5 + (opt.wind || 0.16);
    /* blades on the sunward flank lean into frame */
    const tipx = bx + lean * len * 0.95, tipy = b.by - len;
    const cx2 = bx + lean * len * 0.30, cy2 = b.by - len * 0.62;
    const w = (0.9 + 1.9 * b.r) * grow * (W / 2048);

    /* shade: facing the key light, plus exposure to the sky at the crest */
    const dx = tipx - bx, dy = tipy - b.by, il = 1 / Math.hypot(dx, dy);
    const ndl = sat((-(dx * il) * LIGHT[0] - (dy * il) * LIGHT[1]) * 0.5 + 0.5);
    const open = Math.pow(1 - b.depth, 1.35);
    const l = 0.10 + 0.46 * open + 0.34 * ndl * open;
    const warm = sat(open * 1.25 - 0.42) * (opt.warm !== undefined ? opt.warm : 1);
    const r = (10 + 78 * l + 14 * warm) * (opt.tintR || 1);
    const g = (16 + 106 * l + 40 * warm) * (opt.tintG || 1);
    const bl = (12 + 80 * l + 34 * warm) * (opt.tintB || 1);
    x.fillStyle = hex(r, g, bl);
    x.beginPath();
    x.moveTo(bx - w, b.by);
    x.quadraticCurveTo(cx2 - w * 0.35, cy2, tipx, tipy);
    x.quadraticCurveTo(cx2 + w * 0.35, cy2, bx + w, b.by);
    x.closePath(); x.fill();
  }

  /* --- grade: bounce from the moon, cold sky on the crest ----------- */
  x.globalCompositeOperation = 'source-atop';
  const rb = x.createLinearGradient(opt.bounceFrom || W, 0, opt.bounceTo || 0, 0);
  rb.addColorStop(0, 'rgba(52,120,150,.24)'); rb.addColorStop(0.55, 'rgba(40,92,116,.07)'); rb.addColorStop(1, 'rgba(0,0,0,0)');
  x.fillStyle = rb; x.fillRect(0, 0, W, H);
  const sk = x.createLinearGradient(0, H - H * peak * 1.05, 0, H);
  sk.addColorStop(0, 'rgba(146,182,180,.13)'); sk.addColorStop(0.5, 'rgba(0,0,0,0)'); sk.addColorStop(1, 'rgba(0,0,0,.70)');
...
function texRockCutout(seed, opt) {
  ...
  const blobs = opt.blobs || [[0.30, 0.70, 0.40, 0.34], [0.66, 0.82, 0.38, 0.28], [0.46, 0.96, 0.52, 0.34]];
  ...
  blobs.forEach((b, bi) => {
    ...
    for (let a = 0; a <= 128; a++) {
      const t = a / 128 * TAU;
      const k = 1 + 0.17 * fbm(n, Math.cos(t) * 1.7 + bi * 9, Math.sin(t) * 1.7, 4, 2.1, 0.55);
      const pxx = cx2 + Math.cos(t) * rx * k, py = cy + Math.sin(t) * ry * k;
      if (a === 0) x.moveTo(pxx, py); else x.lineTo(pxx, py);
    }
    x.closePath();
    const g = x.createLinearGradient(cx2 - rx, cy - ry, cx2 + rx * 0.4, cy + ry);
    const s = 1 - bi * 0.16;
    g.addColorStop(0, hex(30 * s, 39 * s, 40 * s));
    g.addColorStop(0.42, hex(14 * s, 19 * s, 20 * s));
    g.addColorStop(1, hex(6, 9, 9));
    x.fillStyle = g; x.fill();
    x.save(); x.clip();
    x.globalCompositeOperation = 'overlay'; x.globalAlpha = 0.62;
    x.drawImage(fbmCanvas(768, 512, seed + bi * 31, 6, 4, 1.1), 0, 0, W, H);
    x.globalAlpha = 1; x.globalCompositeOperation = 'source-over';
    /* facets */
    for (let f = 0; f < 26; f++) {
      x.beginPath();
      const fx = cx2 + (rnd() - 0.5) * rx * 2, fy = cy + (rnd() - 0.5) * ry * 1.6;
      x.moveTo(fx, fy);
      for (let v = 0; v < 3; v++) x.lineTo(fx + (rnd() - 0.5) * rx * 0.8, fy + (rnd() - 0.5) * ry * 0.6);
      x.closePath();
      x.fillStyle = rnd() > 0.5 ? 'rgba(255,255,255,.030)' : 'rgba(0,0,0,.14)';
      x.fill();
    }
    ...
    x.beginPath();
    for (let a = 0; a <= 60; a++) {
      const t = Math.PI + a / 60 * Math.PI * 0.78;
      const k = 1 + 0.17 * fbm(n, Math.cos(t) * 1.7 + bi * 9, Math.sin(t) * 1.7, 4, 2.1, 0.55);
      const pxx = cx2 + Math.cos(t) * rx * k * 0.97, py = cy + Math.sin(t) * ry * k * 0.97;
      if (a === 0) x.moveTo(pxx, py); else x.lineTo(pxx, py);
    }
    x.stroke();
    x.restore();
  ...
  for (let i = 0; i < 2600; i++) {
    const pxx = rnd() * W, py = H - Math.pow(rnd(), 1.7) * H * 0.5;
    x.fillStyle = 'rgba(' + ((40 + rnd() * 40) | 0) + ',' + ((60 + rnd() * 46) | 0) + ',34,' + (0.05 + rnd() * 0.22) + ')';
    x.fillRect(pxx, py, 1.6 + rnd() * 2.4, 3 + rnd() * 9);
  }
  const rb = x.createLinearGradient(W, 0, 0, 0);
  rb.addColorStop(0, 'rgba(58,124,158,.16)'); rb.addColorStop(0.7, 'rgba(0,0,0,0)');
  x.fillStyle = rb; x.fillRect(0, 0, W, H);
  x.globalCompositeOperation = 'source-over';
  return c;
}
...
function texBranchCutout(seed) {
  ...
  function bough(pxx, py, ang, len, wid, depth) {
    const steps = 7;
    let cx2 = pxx, cy = py, a = ang;
    ...
    if (depth < 4 && len > 34) {
      const k = depth === 0 ? 3 : 2;
      for (let i = 0; i < k; i++) bough(cx2, cy, a + (rnd() - 0.5) * 1.25, len * (0.58 + rnd() * 0.18), wid * 0.58, depth + 1);
    } else {
      for (let i = 0; i < 9; i++)
        leaves.push([cx2 + (rnd() - 0.5) * 78, cy + (rnd() - 0.5) * 78, rnd() * TAU, 12 + rnd() * 20, rnd()]);
    }
  }
  bough(W * 1.02, H * 0.06, Math.PI * 0.78, 420, 26, 0);
  bough(W * 0.86, -H * 0.02, Math.PI * 0.62, 330, 18, 1);
...
/* ================================================================= 4 · gl
   Everything below needs a THREE namespace, so it lives inside the factory.
   The host imports three dynamically and hands it in, which keeps three out
   of this site's main bundle exactly as the constellation world did. */
export function createKageWorld(options) {
  const THREE = options.THREE;
  const canvas = options.canvas;
  const REDUCE = !!options.reduce;
  const COARSE = !!options.coarse;
  ...
  /* The layout viewport, not the window. On mobile `innerWidth` follows the
     ...
     while `clientWidth` tracks what CSS actually lays out against. */
  ...
  const LOW = COARSE;
  const WANT_POST = !LOW;
  const WANT_SHADOW = !LOW;
  const DPR_CAP = LOW ? 1.15 : 1.35;
  ...
  const renderer = new THREE.WebGLRenderer({
    canvas: canvas, antialias: !WANT_POST, alpha: false,
  ...
  /* Density puts the hall back in the air; the colour is what decides how
     dark it is back there. At this depth most of the building's pixel is fog
     rather than its own material. And it reaches by distance, so the
     foreground does not follow it down. */
  scene.fog = new THREE.FogExp2(0x070b12, 0.0168);
  ...
  /* HDR-bright emitters: values above 1 survive because the scene renders
     into a half-float buffer, and they are what the bloom feeds on */
  const hdr = (r, g, b) => new THREE.Color().setRGB(r, g, b);

  /* A {map, normal, rough} set on a standard material at one tiling. The
     ...
     roughness stays at 1: the map carries the variation. */
  function surface(t, rep, o) {
    ...
    const m = new THREE.MeshStandardMaterial({
      ...
      normalScale: new THREE.Vector2(o.normal === undefined ? 0.8 : o.normal,
  ...
  /* The board and block libraries are generated once and shared.
     buildLantern alone is called six times, and a granite per lantern is six
     pointless canvases and six normal-map passes at load. */
  const LIB = {};
  ...
  function sweepPoly(points, profile) {
    ...
    [0, segs - 1].forEach((ring, k) => {
      ...
      pos.push(p.x, p.y, p.z); nor.push(0, 0, k ? 1 : -1); uv.push(0.5, 0.5);
      for (let j = 0; j < np; j++) {
        const a = ring * np + j, b = ring * np + (j + 1) % np;
        if (k) idx.push(base, a, b); else idx.push(base, b, a);
      }
    });
  ...
  /* A temple roof. The height is a concave function of the Chebyshev
     distance from the ridge out to the eave, and the last fifth of that run
     flares back upward — that lift at the corners is the whole silhouette.
     Emitted solid: a top shell, an underside shell, and a fascia stitching
     the two rims. */
  function roofGeo(A, B, R, Hr, thick, flare) {
    const NX = 52, NZ = 34, FL = flare === undefined ? 0.30 : flare, e = 1e-3;
    /* The lift has to be weighted toward the corners. Applied evenly along
       the eave it produces a raised rim the whole way round and the roof
       reads as a saucer; concentrated where the two runs meet, it reads as a
       temple. */
    const hAt = (x, z) => {
    ...
    for (let k = 0; k < 2; k++) {
      ...
      for (let j = 0; j < NZ; j++) for (let i = 0; i < NX; i++) {
        ...
        if (k) idx.push(a, c, b, a, d, c); else idx.push(a, b, c, a, c, d);
      }
    }
    /* fascia: its own ring of vertices so the eave keeps a sideways normal */
    const per = [];
  ...
  const STAIR_RUN = 0.55;        /* -> the flight tops out at z = -33       */
  const STAIR_W = 8.4;
  ...
  function buildShell() {
    ...
    const wallMat = new THREE.MeshStandardMaterial({
      ...
      roughness: 0.78, metalness: 0.05, color: 0x525c60,
    });
    /* The court is an open mountain approach: nothing encloses the frame but
       the night, the ridge and the fog. */
    ...
    const floorMat = new THREE.MeshStandardMaterial({
      ...
      /* wet stone: rough, barely metallic, and lit rather than reflecting */
      roughness: 0.74, metalness: 0.06, color: 0x69757a,
    });
    ...
    /* The podium and the flight use their own material — deliberately matte:
       with the floor's metalness the forty risers strobed as the camera
       drifted. */
    const platMat = new THREE.MeshStandardMaterial({
      ...
      normalScale: new THREE.Vector2(0.18, 0.18), roughness: 0.93, metalness: 0.02, color: 0x58636a,
    });
    ...
    cope.position.set(0, PODIUM - 0.17, -45); cope.receiveShadow = true; scene.add(cope);

    /* The flight. Forty risers merged into one buffer — as separate meshes it
       is forty draw calls for a shape nobody ever gets close to. */
    const treads = [], cheeks = [];
    for (let i = 0; i < STEPS; i++) {
      const y = (i + 1) * (PODIUM / STEPS), z = STAIR_Z0 - (i + 0.5) * STAIR_RUN;
      const w = STAIR_W + (STEPS - i) * 0.052;               /* splayed at the foot */
      treads.push(new THREE.BoxGeometry(w, PODIUM / STEPS + 0.04, STAIR_RUN + 0.04)
        .translate(0, y - (PODIUM / STEPS) / 2, z));
      [-1, 1].forEach((s) => cheeks.push(new THREE.BoxGeometry(0.9, 1.5, STAIR_RUN + 0.06)
        .translate(s * (w / 2 + 0.45), y - 0.3, z)));
    }
  ...
  function buildTemple() {
    ...
    /* One board library, sampled at two scales: the hall's flanks want the
       grain long and lying down, the columns want it running their own length. */
    ...
    const gold   = new THREE.MeshStandardMaterial({ color: 0x8f6f2e, roughness: 0.38, metalness: 0.78 });
    const rf = lib('roof', () => texRoof());
    /* Nearly a silhouette: the roof carries almost no colour of its own, and
       what reads on it is relief rather than tone. */
    const tileMat = surface(rf, [1, 1], { color: 0x2b343a, roughness: 0.74, metalness: 0.10, normal: 1.4 });
    ...
    /* The block-and-arm band that carries every eave. The pieces are merged,
       so they carry absolute coordinates — TEMPLE_Z has to go in here. */
    function brackets(halfW, halfD, y, step) {
      const parts = [];
      for (let x = -halfW; x <= halfW + 1e-3; x += step) {
        [-halfD, halfD].forEach((dz) => {
          parts.push(new THREE.BoxGeometry(0.34, 0.46, 0.34).translate(x, y, TEMPLE_Z + dz));
          parts.push(new THREE.BoxGeometry(0.92, 0.17, 0.24).translate(x, y + 0.30, TEMPLE_Z + dz));
        });
      }
      for (let dz = -halfD + step; dz < halfD - 1e-3; dz += step) {
        [-halfW, halfW].forEach((x) => {
          parts.push(new THREE.BoxGeometry(0.34, 0.46, 0.34).translate(x, y, TEMPLE_Z + dz));
          parts.push(new THREE.BoxGeometry(0.24, 0.17, 0.92).translate(x, y + 0.30, TEMPLE_Z + dz));
        });
      }
    ...
    /* ---- ground storey: five bays of lit lattice between charred columns -- */
    ...
    const sill = new THREE.Mesh(new THREE.BoxGeometry(14.6, 0.40, 8.9), post);
    sill.position.set(0, F + 0.20, TEMPLE_Z); g.add(sill);
    ...
    const deck = new THREE.Mesh(new THREE.BoxGeometry(12.4, 0.26, 8.4), post);
    deck.position.set(0, F + 7.72, TEMPLE_Z); g.add(deck);
    [0.10, 0.94].forEach((dy) => [4.2, -4.2].forEach((dz) => {
      const r = new THREE.Mesh(new THREE.BoxGeometry(12.4, 0.17, 0.17), post);
      r.position.set(0, F + 7.85 + dy, TEMPLE_Z + dz); g.add(r);
    }));
    for (let i = 0; i <= 16; i++) {
      const b = new THREE.Mesh(new THREE.BoxGeometry(0.10, 1.05, 0.10), post);
      b.position.set(-6.2 + i * 0.775, F + 8.42, TEMPLE_Z + 4.2); g.add(b);
    }
    for (let i = 0; i < 4; i++)
    ...
    const plqIn = new THREE.Mesh(new THREE.BoxGeometry(1.52, 0.98, 0.18), timber);
    plqIn.position.set(0, F + 9.7, TEMPLE_Z + 3.16); g.add(plqIn);
    brackets(5.2, 3.3, F + 11.2, 1.30);

    /* ---- the main roof: the silhouette the whole scene hangs on ---------- */
    ...
    const ridge = new THREE.Mesh(new THREE.BoxGeometry(7.6, 0.60, 1.05), tileMat);
    ridge.position.set(0, F + 17.10, TEMPLE_Z); g.add(ridge);
    [-1, 1].forEach((s) => {
      const oni = new THREE.Mesh(new THREE.ConeGeometry(0.46, 1.15, 4), tileMat);
    ...
    const spill = new THREE.Mesh(new THREE.PlaneGeometry(30, 16),
      new THREE.MeshBasicMaterial({ map: tx(texGlow('rgba(168,214,232,.80)', 'rgba(96,160,196,.24)')),
        transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, fog: false, opacity: 0.30 }));
    ...
    /* the band of mist that separates the hall from the trees behind it — the
       one thing that stops the whole background reading as a flat cut-out */
    const mist = new THREE.Mesh(new THREE.PlaneGeometry(64, 20),
      new THREE.MeshBasicMaterial({ map: tx(texGlow('rgba(150,178,190,.62)', 'rgba(104,138,154,.20)')),
        transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, fog: false, opacity: 0.17 }));
  ...
  function buildMoon() {
    const disc = new THREE.Mesh(new THREE.PlaneGeometry(MOON.r * 2, MOON.r * 2),
      /* Kage grades this disc as a blood moon — red, with green and blue
         level under it. That is the one colour in the scene this site has no
         token for, and a red moon over a navy audit reads as two designs in
         one frame. The albedo is authored neutral, so the whole grade is
         this one line: retuned to --data (#7fafe0), the blue the site
         already uses for anything instrument-like. Written in linear,
         because the map is decoded out of sRGB before this multiplies it. */
      new THREE.MeshBasicMaterial({ map: tx(texMoon()), color: hdr(0.80, 1.45, 2.45),
    ...
    const halo = new THREE.Mesh(new THREE.PlaneGeometry(MOON.r * 6.4, MOON.r * 6.4),
      new THREE.MeshBasicMaterial({ map: tx(texGlow('rgba(150,196,236,.90)', 'rgba(70,128,190,.26)')),
  ...
  /* On a tall frame the rig steps back and its horizontal reach narrows,
     which would slide the moon off the right edge — draw it in by as much. */
  function placeMoon() {
    if (!WORLD.moon) return;
    const x = MOON.x * (1 - 0.40 * aspectFix());
  ...
  function buildTorii() {
    /* the weathering costs the gate a lot of its chroma, so the tint puts it
       back: red still lifted past green and blue, or it comes out of the
       grade as brick — but pulled down to --flag (#d9705c), the muted
       terracotta this site already stamps a null result with, rather than
       the source's full vermilion */
    const lac = surface(lib('lacquer', () => texLacquer()), [2, 2],
      { color: hdr(1.34, 0.86, 0.78), roughness: 0.92, metalness: 0.05, normal: 0.75 });
    const gold = new THREE.MeshStandardMaterial({ color: 0x7a5d2a, roughness: 0.50, metalness: 0.60 });
    const g = new THREE.Group();
    ...
    [-1, 1].forEach((s) => {
      ...
      const collar = new THREE.Mesh(new THREE.CylinderGeometry(0.345, 0.345, 0.26, 26), gold);
      collar.position.set(s * SPAN, BASE + H - 1.42, 0); g.add(collar);
      /* the ornate bracket where the beams cross the column */
      const br = new THREE.Mesh(new THREE.BoxGeometry(0.86, 0.30, 0.86), gold);
      br.position.set(s * SPAN, BASE + H - 0.72, 0); g.add(br);
      const br2 = new THREE.Mesh(new THREE.BoxGeometry(0.60, 0.52, 0.60), gold);
      br2.position.set(s * SPAN, BASE + H - 0.34, 0); g.add(br2);
      [-1, 1].forEach((d) => {
        const wing = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.70, 0.18), gold);
        wing.position.set(s * SPAN + d * 0.40, BASE + H - 0.62, 0.30); g.add(wing);
        const wing2 = wing.clone(); wing2.position.z = -0.30; g.add(wing2);
      });
    });
    ...
    const gak = new THREE.Mesh(new THREE.BoxGeometry(0.46, 1.22, 0.40), lac);
    gak.position.set(0, BASE + H + 0.10, 0); g.add(gak);
    const gakG = new THREE.Mesh(new THREE.BoxGeometry(0.60, 0.18, 0.50), gold);
    gakG.position.set(0, BASE + H + 0.68, 0); g.add(gakG);
    ...
    const shimaki = new THREE.Mesh(sweepPoly(beamPath(5.05, 0.40, 2.6),
      [[-0.30, -0.19], [0.30, -0.19], [0.32, 0.06], [0.28, 0.21], [-0.28, 0.21], [-0.32, 0.06]]), lac);
    shimaki.position.set(0, BASE + H - 0.52, 0); shimaki.castShadow = true; g.add(shimaki);
    ...
    const cap = new THREE.Mesh(sweepPoly(beamPath(5.95, 0.62, 2.4),
      [[-0.48, 0.22], [0.48, 0.22], [0.48, 0.34], [-0.48, 0.34]]),
      new THREE.MeshStandardMaterial({ color: 0x120c0c, roughness: 0.42, metalness: 0.14 }));
    cap.position.set(0, BASE + H + 0.96, 0); g.add(cap);

    /* The lowest thing on the gate is the foot of each column, and it starts
       at BASE — so anchoring the group at y=0 would leave the whole torii
       hanging BASE*scale above the court. Drop it by exactly that. */
    const GS = 0.72;
  ...
  function buildLantern(x, z, s, y) {
    ...
    const dark  = lib('lanternDark', () => surface(postWood(), [0.9, 0.9],
      { color: 0xd8c2b6, metalness: 0.1, normal: 0.7 }));
    const g = new THREE.Group();
    ...
    /* Not the source's ember. A lantern is the one thing in this scene that
       is deliberately, individually lit — which is what a ◆ is on this site,
       so they burn --seal (#3dd698) and the walk up the flight is a row of
       verified marks. */
    const paneMat = new THREE.MeshBasicMaterial({ color: hdr(0.30, 2.10, 1.35), fog: false, toneMapped: false });
    ...
    const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.66, 0.70, 0.07, 4, 1), stone);
    brim.position.y = 1.93; brim.rotation.y = Math.PI / 4; g.add(brim);
    const knob = new THREE.Mesh(new THREE.SphereGeometry(0.09, 12, 10), stone);
    knob.position.y = 2.28; g.add(knob);
    const tip = new THREE.Mesh(new THREE.ConeGeometry(0.055, 0.16, 10), stone);
    tip.position.y = 2.4; g.add(tip);

    const glow = new THREE.Mesh(new THREE.PlaneGeometry(3.4, 3.4),
      new THREE.MeshBasicMaterial({ map: tx(texGlow('rgba(120,246,196,.9)', 'rgba(40,200,146,.28)')),
    ...
    const lt = new THREE.PointLight(0x3dd698, 2.4, 9, 2);
  ...
  function buildMaple(seed, x, z, scale) {
    ...
    const trunk = new THREE.Mesh(mergeGeos(parts),
      new THREE.MeshStandardMaterial({ color: 0x171413, roughness: 0.94, metalness: 0.0, side: THREE.DoubleSide }));
    trunk.castShadow = true;

    const leafGeo = new THREE.PlaneGeometry(0.36, 0.36);
    const leafMat = new THREE.MeshStandardMaterial({
      map: tx(texLeaf()), color: 0x2f1c16, alphaTest: 0.42, side: THREE.DoubleSide,
      roughness: 0.86, metalness: 0, emissive: 0x1a0a06, emissiveIntensity: 0.12,
    });
    const per = LOW ? 5 : 9;
  ...
  function buildRocks() {
    const mat = new THREE.MeshStandardMaterial({ color: 0x141a1c, roughness: 0.46, metalness: 0.10 });
    const rnd = mulberry32(404), n = noise2D(88);
    const spots = [[-6.2, -5.4, 0.95], [-7.4, -2.1, 0.7], [6.6, -7.2, 0.8], [8.0, -4.0, 1.05],
                   [-9.0, -8.4, 1.15], [9.6, -9.2, 0.9], [3.4, -7.8, 0.55], [-3.2, -8.6, 0.6]];
    spots.forEach((s, i) => {
  ...
  function cutoutMaterial(canvasEl, sway) {
    ...
    mat.onBeforeCompile = (sh) => {
      sh.uniforms.uT = WORLD.uT;
      sh.uniforms.uSway = { value: sway || 0.07 };
      sh.vertexShader = 'uniform float uT;\nuniform float uSway;\n' + sh.vertexShader.replace(
        '#include <begin_vertex>',
        '#include <begin_vertex>\n' +
        'float h = uv.y;\n' +
        'float ph = position.x*0.30;\n' +
        'transformed.x += sin(uT*0.85 + ph)*uSway*h*h;\n' +
        'transformed.y += cos(uT*0.62 + ph*1.7)*uSway*0.35*h*h;\n');
      /* These plates are drawn to the edge of their own canvas — the grass
         runs straight off the left and the bottom — so every sheet ends on a
         hard straight line wherever its plane stops. Feathering the alpha at
         the edges lets each one dissolve into the scene instead of being cut
  ...
  /* Each plate is its own job: a single grass cut-out draws thousands of
     quads, so building all six in one call is the largest uninterruptible
     block in the whole boot. The counts are also well below the source's —
     Kage's foreground is the subject of its page, where here it sits behind
     a scrimmed document and nobody is reading the individual blades. */
  const FG_DEFS = [
    /* name        canvas thunk                                                                                 x     y     z     w     h    sway */
    ['grassFar',  () => texGrassCutout(101, { crest: 0.30, peak: 0.46, wide: 0.42, blades: LOW ? 2600 : 5200, len: 34, warm: 0.70, crest2: 0.46, crest3: 0.40 }),
                  -3.4,  2.85, -0.4, 24, 12, 0.040],
    ['rockLeft',  () => texRockCutout(404, { blobs: [[0.24, 0.78, 0.48, 0.38], [0.58, 0.94, 0.46, 0.30], [0.84, 1.06, 0.42, 0.28]] }),
                  -7.2,  2.55,  1.8, 17, 11.3, 0.010],
    ['grassMid',  () => texGrassCutout(202, { crest: 0.60, peak: 0.55, wide: 0.40, blades: LOW ? 3000 : 6000, len: 40, warm: 0.95, crest2: -0.44, crest3: 0.34 }),
                   3.9,  2.86,  3.6, 15,  7.5, 0.062],
    ['grassNear', () => texGrassCutout(303, { crest: 0.42, peak: 0.62, wide: 0.48, blades: LOW ? 3500 : 7000, len: 50, warm: 0.85, crest2: 0.42, crest3: 0.44 }),
                  -1.9,  2.92,  5.6, 12,  6, 0.092],
    ['bough',     () => texBranchCutout(505), 4.6, 2.95, 6.4, 13, 8.7, 0.045],
    ['rockNear',  () => texRockCutout(606, { blobs: [[0.18, 0.86, 0.54, 0.44], [0.54, 1.02, 0.50, 0.36], [0.86, 0.92, 0.40, 0.32]] }),
                  -6.6,  1.70,  6.0, 9, 6, 0.008],
  ];
  function buildForegroundLayer(i) {
  ...
  function buildAtmosphere() {
    ...
    for (let i = 0; i < (LOW ? 4 : 6); i++) {
      const s = 12 + rnd() * 15;
      const h = new THREE.Mesh(new THREE.PlaneGeometry(s, s * 0.55),
        new THREE.MeshBasicMaterial({ map: hazeTex, transparent: true, blending: THREE.AdditiveBlending,
          depthWrite: false, fog: false, opacity: 0.05 + rnd() * 0.07 }));
    ...
    /* embers around the lanterns and the disc */
    const N = LOW ? 220 : 460;
    const pos = new Float32Array(N * 3), seed = new Float32Array(N);
    for (let i = 0; i < N; i++) {
      pos[i * 3] = (rnd() - 0.5) * 30; pos[i * 3 + 1] = rnd() * 11; pos[i * 3 + 2] = -26 + rnd() * 36;
      seed[i] = rnd();
    }
    const g = new THREE.BufferGeometry();
    ...
    const emb = new THREE.Points(g, new THREE.ShaderMaterial({
      uniforms: { uT: WORLD.uT, uTex: { value: tx(texGlow('rgba(190,250,224,1)', 'rgba(80,214,164,.35)')) },
        uSize: { value: vpH() * 0.5 } },
      ...
      fragmentShader:
        ...
        ' gl_FragColor = vec4(t.rgb*vec3(0.40,1.42,1.06), t.a*vA*0.75); }',
    }));
    ...
    if (!LOW) {
      ...
      for (let i = 0; i < D; i++) {
        const x = (rnd() - 0.5) * 40, z = -30 + rnd() * 34, y = rnd() * 17, sp = 7 + rnd() * 9, ln = 0.30 + rnd() * 0.55;
        for (let k = 0; k < 2; k++) {
          const o = (i * 2 + k) * 3;
    ...
    rg2.addColorStop(0, 'rgba(255,255,255,0)'); rg2.addColorStop(0.62, 'rgba(255,255,255,.55)');
    rg2.addColorStop(0.86, 'rgba(255,255,255,.22)'); rg2.addColorStop(1, 'rgba(255,255,255,0)');
    ...
    for (let i = 0; i < (LOW ? 6 : 13); i++) {
      ...
      r.userData = { t: rnd() * 4, x: (rnd() - 0.5) * 22, z: -8 + rnd() * 22, sp: 0.5 + rnd() * 0.5 };
      scene.add(r); WORLD.ripples.push(r);
    }
  }

  /* ====================================================== 6b · the leaf fall
     The maples shed all the way through. Every leaf keeps a real world
     position so its drift is honest, but any leaf that ends up further than
     LEAF_R from the camera in x or z is wrapped back across it. That way the
     count is what one screenful needs instead of what filling the whole
     ...
     comes back — which is why these are instanced quads and not points. */
  ...
  function buildLeafFall() {
    ...
    const mat = new THREE.MeshStandardMaterial({
      ...
      color: 0x4a2318, roughness: 0.84, metalness: 0,
      /* still red-led, or the composite's tone-map turns it pink — but at
         --flag's warmth rather than the source's arterial red */
      emissive: 0x8a3826, emissiveIntensity: 0.72,
    });
    const inst = new THREE.InstancedMesh(new THREE.PlaneGeometry(0.40, 0.40), mat, N);
    ...
    for (let i = 0; i < N; i++) list.push({
      x: (rnd() - 0.5) * 2 * LEAF_R, z: (rnd() - 0.5) * 2 * LEAF_R, y: rnd() * 26,
      fall: 0.5 + rnd() * 0.9,
      sway: 0.45 + rnd() * 1.5, swayPh: rnd() * TAU, swayAmp: 0.30 + rnd() * 0.95,
      spin: (rnd() - 0.5) * 2.6, roll: rnd() * TAU, rollSp: 0.5 + rnd() * 2.0,
      tilt: rnd() * TAU, s: 0.55 + rnd() * 0.9,
    });
    scene.add(inst);
    WORLD.leaves = { mesh: inst, list: list, seeded: false };
  }
  ...
  function updateLeaves(dt, clock) {
    ...
    /* Respawn ahead of the rig, not around it. A band centred on the camera
       spends nearly all of itself behind and beside the frustum — of a couple
       of hundred leaves only a dozen were ever on screen. Recycling them into
       the cone the camera is actually looking down puts the same count in
       frame several times over, and costs nothing. */
    ...
    for (let i = 0; i < L.length; i++) {
      ...
      LEAF_P.set(l.x + sw * l.swayAmp, l.y,
                 l.z + Math.cos(clock * l.sway * 0.7 + l.swayPh) * l.swayAmp * 0.6);
  ...
  /* ==================================================== 6c · the cursor wisps
     ...
     camera space through the frustum's own half-height. Unprojecting onto a
     world plane each frame would instead pin the trail to the court, and the
     rig is always drifting — the wisps would swim across the screen whenever
     the camera moved rather than staying under the hand.

     Emission is by distance travelled, not elapsed time. A slow hand then
     lays a continuous drift and a fast one throws the motes apart, which is
     how a moving source actually sheds them. */
  const WISP_D = 3.4;
  const WISP = { list: [], i: 0, acc: 0, ex: 0, ey: 0, lx: 0, ly: 0, idle: 0, seen: false, mesh: null };
  function buildWisps() {
    if (COARSE) return;
    /* The motes are round, so nothing here is oriented. They are also small —
       a few pixels of core inside a faint halo — which is what lets the count
       go up without the additive fill a screenful of big sprites costs. */
    const N = LOW ? 90 : 190;
    const g = new THREE.BufferGeometry();
  ...
  function updateWisps(dt, clock) {
    ...
    const spawn = (x, y, a, weak) => {
      const i = WISP.i; WISP.i = (i + 1) % N;      /* slot and state must pair:
         advancing first would put the position in the next slot and the life
         in this one, and every mote appears where the one before it started */
      const w = L[i], k = i * 3;
      /* The plane the trail hangs on is only about 2.2 units tall, so the
         scatter has to be read against that. */
      P[k] = x + (Math.random() + Math.random() - 1) * 0.30;
      P[k + 1] = y + (Math.random() + Math.random() - 1) * 0.30;
    ...
    while (WISP.acc >= STEP && guard++ < 14) {
      WISP.acc -= STEP;
      /* lay each one at the distance along the segment it is actually owed,
         so a flick draws a spaced-out line instead of a clump at one end */
      const t = moved > 1e-6 ? Math.min(1, guard * STEP / moved) : 0;
      spawn(WISP.lx + dx * t, WISP.ly + dy * t, ang, false);
    }
    /* the faintest breath when the hand is still, so the cursor keeps an
       aura. Rarely, and barely buoyant: emit often here and a stationary
       pointer grows a permanent column of smoke up the middle of the frame. */
    WISP.idle += dt;
    ...
    for (let i = 0; i < N; i++) {
      ...
      /* light damping: they have to coast to carry the scatter outward */
      w.vx *= 1 - 0.5 * dt; w.vy = w.vy * (1 - 0.5 * dt) + 0.022 * dt;    /* it rises */
      A[i] = smooth(0, 0.12, u) * (1 - smooth(0.22, 1, u)) * 0.9;
      S[i] = w.sz * (1 + u * 0.55);       /* a mote softens, it does not swell */
    }
  ...
  function buildLights() {
    ...
    const moonKey = new THREE.DirectionalLight(0x7fafe0, 0.58);
    moonKey.position.set(26, 30, -60);
    moonKey.target.position.set(0, 8, -40); scene.add(moonKey.target);
    scene.add(moonKey);

    /* The hall's own lamps. They light the podium and the top of the flight
       and nothing else — a range that reaches the roof turns the whole
       building into a paper lantern and the silhouette disappears. */
    const hallL = new THREE.PointLight(0x8fc8dc, 2.3, 15, 2);
    hallL.position.set(0, PODIUM + 1.2, TEMPLE_Z + 8.6); scene.add(hallL); WORLD.hallLight = hallL;
    [-1, 1].forEach((s) => {
      const w = new THREE.PointLight(0x88c0d6, 2.2, 11, 2);
      w.position.set(s * 11.4, PODIUM + 1.2, TEMPLE_Z + 5.6); scene.add(w);
    });

    /* the moon throws almost nothing, but a trace of its own blue high on
       the right keeps it attached to the scene instead of floating on top */
    const moonL = new THREE.PointLight(0x4f86bb, 2.6, 46, 2);
    ...
    /* the flight is the spine of the composition — without a lamp of its own
       it falls into the same black as the ground and the eye has nothing to
       climb */
    const stairL = new THREE.PointLight(0x63c6a4, 4.2, 17, 2);
  ...
  function initPost() {
    ...
    POST.bright = new THREE.ShaderMaterial({
      uniforms: { tS: { value: null }, uThr: { value: 0.86 }, uKnee: { value: 0.50 } },
    ...
    POST.comp = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uT: { value: 0 }, uBloom: { value: 0.34 }, uCA: { value: 1 }, uGrain: { value: 0.020 },
        uVig: { value: 1 }, uExp: { value: 0.62 }, uFade: { value: 1 }, uSat: { value: 1.05 },
      },
      vertexShader: QUAD_VS,
      fragmentShader:
        ...
        ' c = mix(c, c*vec3(0.982,1.004,1.038), smoothstep(0.50,1.0,l)*0.26);\n' + /* cool highs   */
        ...
        /* A touch of contrast, applied after the encode rather than before it:
           in linear the same curve mostly crushes the shadows, and this frame
           is nearly all shadow. Pivoted low, at 0.30, so it deepens the night
  ...
  function renderPost() {
    ...
    POST.up.blending = THREE.AdditiveBlending; POST.up.uniforms.uAmt.value = 0.52;
  ...
  /* ================================================= 8 · the camera rig
     The authored waypoints, unchanged. The host drives `prog` as a 0..1
     fraction of its own scroll rather than from Kage's [data-cam] sections,
     so however many chapters this site has, the walk from the gate to the
     hall spans exactly the same spline. */
  const CAM = [
    { p: [  0.0, 4.05, 13.6 ], t: [  0.0, 6.60, -18.0 ], fov: 36 },  /* 0 the approach */
    { p: [ -5.6, 2.35, 11.6 ], t: [  1.2, 5.60, -14.0 ], fov: 48 },  /* 1 the sanmon   */
    { p: [  1.2, 3.60,  2.2 ], t: [ -0.6, 7.50, -22.0 ], fov: 40 },  /* 2 the gardens  */
    { p: [  5.2, 2.10, -3.4 ], t: [ -2.6, 7.00, -20.0 ], fov: 46 },  /* 3 the craft    */
    { p: [  0.0, 7.60, -16.0 ], t: [  0.0, 13.0, -40.0 ], fov: 42 }, /* 4 afterlight   */
    { p: [  0.0, 10.5, -20.0 ], t: [  0.0, 3.00, -34.0 ], fov: 46 }, /* 5 the colophon */
  ];
  const RIG = { prog: 0, smooth: 0, mx: 0, my: 0, tmx: 0, tmy: 0, intro: 0 };
  const curveP = new THREE.CatmullRomCurve3(CAM.map((c) => new THREE.Vector3(c.p[0], c.p[1], c.p[2])), false, 'catmullrom', 0.42);
  const curveT = new THREE.CatmullRomCurve3(CAM.map((c) => new THREE.Vector3(c.t[0], c.t[1], c.t[2])), false, 'catmullrom', 0.42);
  ...
  /* ==================================================== 9 · the machine */
  let clock = 0, fadeIn = 0, running = false, tPrev = 0, raf = 0, FRAME = 0;
  let introT0 = 0, booted = false;
  ...
  function resize() {
    ...
    if (POST.scene) {
      const want = (!LOW && PERF.scale > 0.78) ? 2 : 0;
  ...
  function updateWorld(dt) {
    ...
    if (WORLD.hallHalo) WORLD.hallHalo.material.opacity = 0.30 + Math.sin(clock * 0.6) * 0.035;
    if (WORLD.hallLight) WORLD.hallLight.intensity = 3.4 * (1 + Math.sin(clock * 0.43) * 0.045);
    /* the moon only breathes — the haze in front of it is what actually moves */
    if (WORLD.moonHalo) WORLD.moonHalo.material.opacity = 0.44 + Math.sin(clock * 0.34) * 0.05;
    if (WORLD.lanternLights) WORLD.lanternLights.forEach((l, i) => {
      l.intensity = 2.6 * (0.86 + 0.22 * Math.sin(clock * (2.3 + i * 0.7) + i * 2.1) + 0.1 * pulse);
    });
    ...
    if (WORLD.ripples) WORLD.ripples.forEach((r) => {
      const u = r.userData;
      u.t += dt * u.sp;
      if (u.t > 4) { u.t = 0; u.x = (Math.random() - 0.5) * 22; u.z = -8 + Math.random() * 22; }
      const k = u.t / 4;
      const s = 0.3 + k * 4.2;
      r.scale.set(s, s, 1);
      r.position.set(u.x, 0.02, u.z);
      r.material.opacity = Math.sin(k * Math.PI) * 0.16;
    });
    ...
    updateLeaves(dt, clock);
    updateWisps(dt, clock);
  }
  ...
  function frame(now) {
    ...
    if (JOBS.length) {
      /* One builder per frame. The renderer is not touched until the last
         job has run, so a half-built temple is never shown. */
      const job = JOBS.shift();
      try { job(); } catch (err) { console.error('[kage] build step failed', err); }
      tPrev = now;
      return;
    }
    if (!booted) return;
    const raw = (now - tPrev) / 1000 || 0;
    const dt = Math.min(raw, 0.05);              /* animation never jumps … */
    tPrev = now; clock += dt;
    fadeIn = introT0 ? sat((now - introT0) / 700) : 1;
    ...
    if (clock > 2.2) {
      ...
      if (PERF.n >= 40 || PERF.acc > 0.9) {
        ...
        if (avg > 0.0230 && PERF.scale > 0.55) { PERF.scale = Math.max(0.55, PERF.scale * (avg > 0.05 ? 0.64 : 0.85)); resize(); }
    ...
    if (introT0) RIG.intro = sat((now - introT0) / 1000 / 2.4);
  ...
  /* --------------------------------------------------------------- boot
     The source builds the whole world behind a preloader, stepping a JOBS
     list with setTimeout so the page stays responsive while a few dozen
     procedural canvases are generated. That structure is the reason this
     port is usable at all: built in one call it is a second and a third of
     uninterrupted main thread, which Lighthouse reads as total blocking
     time and a reader feels as a freeze.
  ...
  const JOBS = [
    ...
    () => {
      /* The pairs flanking the flight stand on the rail rather than beside
         it: solve the step under each one from the same constants the treads
         and cheeks are built from. */
    ...
    () => { buildMaple(71, 12.6, -13.0, 1.05); },
    () => { buildMaple(72, -11.8, -9.4, 0.95); },
    () => { buildMaple(73, 9.2, -19.0, 0.82); },
    () => { buildMaple(74, -14.5, -17.5, 1.0); },
    () => { buildMaple(75, 16.5, -6.0, 0.88); },
    () => buildForegroundLayer(0),
    () => buildForegroundLayer(1),
    () => buildForegroundLayer(2),
    () => buildForegroundLayer(3),
    () => buildForegroundLayer(4),
    () => buildForegroundLayer(5),
    () => { buildAtmosphere(); },
    () => { buildLeafFall(); buildWisps(); },
    () => {
      initPost();
      ...
      applyCamera();
      booted = true;
      introT0 = REDUCE ? performance.now() - 4000 : performance.now();
    },
  ];
  ...
  return {
    ...
    setProgress(t) { RIG.prog = clamp(t, 0, 1) * (CAM.length - 1); },
    /** Pointer, in normalised device coordinates. */
    setPointer(nx, ny) { RIG.tmx = clamp(nx, -1, 1); RIG.tmy = clamp(ny, -1, 1); },
    resize: resize,
    start() {
      if (running) return;
      running = true;
